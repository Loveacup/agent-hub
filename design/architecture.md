...
**实时控制协议**：所有生产力 CLI agent lane 都必须支持 `execute + monitor + intervene + gated interrupt + event`。CC 通过 PTY 长会话实现 live follow-up；Codex exec 不支持热输入时也必须显式返回 `unsupported_live_intervention`，并提供 `interrupt + re-exec` 替代路径。
