...
| 6 | review-worker gate-runner + routing registry/policy + 后续 omp-worker | ✅ Slice 1-4 |
...
| `review-worker` | cc-tmux `gate/*.sh` packaged runtime assets + routing registry/policy | 统一客观验收、危险扫描、讨论/退回计数入口；iii runtime smoke 与 route::decide smoke 已通过 | 不自动审批；不修改 cc-tmux gate 原件；不执行生产力任务；route 决策 `execute=false` 且带 `decision_code` |
