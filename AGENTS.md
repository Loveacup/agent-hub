...
| 7 | omp-worker · OMP profile lifecycle manager（registry/discover/render/validate/audit） | 🟡 Slice 1-6（registry + discover + render + apply + validate helpers；Slice 6 = metadata-only audit/event helpers + review-worker OMP lifecycle 路由识别，全部 `execute=false`、带监控契约，omp 运行时 lane 仍未注册） |
