...
│       └── review-worker/      # Phase 6 Slice 1: gate-runner 审核 worker
├── agent-hub-skill/
...
| 7 | omp-worker · OMP profile lifecycle manager（registry/discover/render/validate/audit） | 🟡 Slice 1-7（registry + discover + render + apply + validate helpers；Slice 6 = metadata-only audit/event helpers + review-worker OMP lifecycle 路由识别；Slice 7 = fail-closed runtime entry `src/index.js`（纯 JS、无 fs/子进程/网络、无 `process.env`，execution-shaped/未知请求一律 `execute=false` unsupported，带监控契约），全部 `execute=false`，omp 运行时 lane 仍未注册） |
...
| `review-worker` | cc-tmux `gate/*.sh` packaged runtime assets + routing registry/policy | 统一客观验收、危险扫描、讨论/退回计数入口；iii runtime smoke、route::decide smoke 与 agent.route.decision publish smoke 已通过 | 不自动审批；不修改 cc-tmux gate 原件；不执行生产力任务；route 决策 `execute=false` 且带 `decision_code` |
